...
void ili9341_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_map)
{
	...
	data[0] = ((area->x1 +40)>> 8) & 0xFF;
	data[1] = (area->x1 +40) & 0xFF;
	data[2] = ((area->x2 +40) >> 8) & 0xFF;
	data[3] = (area->x2 +40) & 0xFF;
	ili9341_send_data(data, 4);

	/*Page addresses*/
	ili9341_send_cmd(0x2B);
	data[0] = ((area->y1 +52) >> 8) & 0xFF;
	data[1] = (area->y1 +52) & 0xFF;
	data[2] = ((area->y2 +52) >> 8) & 0xFF;
	data[3] = (area->y2 +52) & 0xFF;
	ili9341_send_data(data, 4);
...
static void ili9341_set_orientation(uint8_t orientation)
{
	...
	//uint8_t data[] = {0x40, 0x78, 0x78, 0x78};
	uint8_t data[] = {0x60, 0x00, 0xc0, 0x78};//00,c0,a0,60
#endif
